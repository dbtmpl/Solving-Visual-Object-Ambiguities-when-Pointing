...
# Load video
# cap = cv.VideoCapture('resources/current_training/video_data/tests/noise_detection_5.webm')
cap = cv.VideoCapture('resources/current_training/video_data/three objects/amb 1/amb1_o3_r1_m.webm')

# Which approach => True: Computer Vision approach, False: GWR
# pointing_estimation = False
pointing_estimation = True
...
complete_frame = cv.resize(first_frame, (f_shape[1] / 2, f_shape[0] / 2))
first_frame = first_frame[0:crops_y, crops_x1:crops_x2]
first_frame = cv.resize(first_frame, ((crops_x2 - crops_x1) / 2, crops_y / 2))

# initialize gwr for predicting
link_data_gwr = "resources/current_training/gwr_data/12_07_hand_shape/trained_gwr/classic_normalized/cross_90_30e/0"
...
while cv.waitKey(30) & 0xFF != ord('q'):
    ...
    complete_frame = cv.resize(frame, (f_shape[1] / 2, f_shape[0] / 2))
    ...
    if ret:
        ...
        if pointing_estimation:
            ...
            for i, d_object in enumerate(detected_objects):
                if d_object is not None:
                    ...
                    object_bb.append(["The " + color_str + " Object", (bb[0], bb[1], bb[2] - bb[0], bb[3] - bb[1]), bb])
        ...
        if tracking_state == "One" or tracking_state == "Two":
            ...
            cnts, skin_binary, thresh, prep_steps = state.search_new_hand_cnts(skin_binary, mask_1, mask_2, mask_result,
            ...
            if cnts is not None:
                ...
                if len(cnts) == 1:
                    ...
                    frame, frame2, tracking_state, thresh = state.tracking_one_hand(frame, cnts, thresh, hand_positions_t0,
                    ...
                    frame, frame2, tracking_state, thresh = state.tracking_two_hands(frame, cnts, thresh, hand_positions_t0,
                                                                             object_bb, mean_aver, gwr,
                                                                             pointing_estimation)
